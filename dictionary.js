...
// Add these to your App.config object
const App = {
    // ...
    config: {
        // ...
    ...
    elements: {
        // ...
        mainContent: document.querySelector('main'), // Get the main element
        studyContainer: null
    },
    // NEW: Make modal functions globally accessible for study-session.js
    showMnemonic,
...
// REFACTORED: This is the new core function
async function startStudySession() {
    ...
    // 1. Save state
    App.config.lastScrollPosition = window.scrollY;
    const studyUrl = `/study.html?words=${encodeURIComponent(App.config.studyList.join(','))}`;
    
    // 2. Update the URL without reloading
    history.pushState({ view: 'study' }, "Study Session", studyUrl);
    
    // 3. Switch the view
    await switchView('study');
    
    // 4. Reset selection mode
    toggleSelectionMode();
}

// NEW: This function handles showing/hiding the views
async function switchView(view) {
    // Ensure the container for the study session exists
    if (!App.elements.studyContainer) {
        App.elements.studyContainer = document.createElement('div');
        App.elements.studyContainer.id = 'study-session-view';
        App.elements.mainContent.appendChild(App.elements.studyContainer);
    }

    if (view === 'study') {
        App.elements.appContainer.style.display = 'none'; // Hide dictionary
        App.elements.studyContainer.style.display = 'block';
        App.elements.studyContainer.innerHTML = '<p style="text-align: center; padding: 50px;">Loading Study Session...</p>';
        try {
            await loadScript('/study-session.js');
            // Call the main function from the now-loaded script
            initializeStudySession(App.elements.studyContainer, App.config.studyList);
        } catch (error) {
            App.elements.studyContainer.innerHTML = `<p>Error loading study session: ${error.message}</p>`;
        }
    } else { // 'dictionary' view
        App.elements.appContainer.style.display = 'block'; // Show dictionary
        App.elements.studyContainer.style.display = 'none';
        // Restore scroll position
        window.scrollTo(0, App.config.lastScrollPosition || 0);
    }
}

// NEW: Add a listener for the browser's back button
window.addEventListener('popstate', (event) => {
    // When the user clicks back from the study session, this will trigger
    if (document.location.pathname.startsWith('/lesson') || document.location.pathname === '/') {
        switchView('dictionary');
    }
});

// Make sure your `document.addEventListener('DOMContentLoaded', ...)` also handles this
document.addEventListener('DOMContentLoaded', () => {
    // ... your existing DOMContentLoaded logic ...

    // Add this to handle the case where a user refreshes on the /study.html page
    if (document.location.pathname.startsWith('/study.html')) {
        // Redirect to the homepage so they can start a session properly
        window.history.replaceState(null, '', '/');
        // Now switch to the dictionary view
        switchView('dictionary');
    }
});
