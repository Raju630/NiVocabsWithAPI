// dictionary.js (Complete Version with Weak Word Fixes)
...
// --- NEW FUNCTION: Specifically for removing from weak list ---
function removeWordFromWeakList(word) {
    App.data.weakWords = App.data.weakWords.filter(w => w !== word);
    saveUserData();
    // Re-render only the weak words list to reflect the change immediately
    renderWeakWordsList();
}

// --- MODIFIED: deleteWord now ONLY handles permanent deletion ---
function deleteWord(word) {
    if (!App.data.deletedWords.includes(word)) {
        App.data.deletedWords.push(word);
    }
    delete App.data.userWords[word];
    // Also ensure it's removed from the weak words data array
    App.data.weakWords = App.data.weakWords.filter(w => w !== word);
    saveUserData();
    updateLiveDictionary();
    // This correctly refreshes the main dictionary view
    resetAndLoadWords();
}
...
function handleWordCardClick(e) {
    ...
    if (actionButton) {
        e.stopPropagation();
        if (actionButton.classList.contains('examples')) {
            showExampleSentences(word);
        } else if (actionButton.classList.contains('mnemonic')) {
            showMnemonic(word);
        } else if (actionButton.classList.contains('edit')) {
            openEditModal(word);
        } else if (actionButton.classList.contains('delete')) {
            // Check which tab is currently active to decide the action
            const weakWordsTab = document.getElementById('weak-words-tab');
            if (weakWordsTab && weakWordsTab.classList.contains('active')) {
                // If on the weak words tab, only remove it from the weak list
                removeWordFromWeakList(word);
            } else {
                // Otherwise (on Dictionary tab), perform a permanent delete
                if (confirm(`Are you sure you want to permanently delete the word "${word}"? This cannot be undone.`)) {
                    deleteWord(word);
                }
            }
        }
...
// ... (No other functions in the file need to be changed. The rest of your code is correct.)
// ... For completeness, the full file content is below, but the key changes are the three functions above.
...
function createWordCard(word) {
    ...
    // This check prevents crashes if a word was deleted but still in the weak list temporarily
    if (!wordData) return document.createDocumentFragment(); 

    const { meaning, category, lesson, en, isUserWord } = wordData;
    if (isUserWord) {
...
async function showExampleSentences(banglaWord) {
    ...
    const submitSentenceRequest = async (fullWordObject) => {
        ...
        try {
            const response = await fetch('/.netlify/functions/request-sentence', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(fullWordObject) });
    ...
    try {
        ...
        if (requestButton) {
            requestButton.addEventListener('click', () => {
                const wordObjectForRequest = { bangla: banglaWord, japanese: wordData.meaning, english: wordData.en };
...
function renderApp() {
    if (!App.elements.appContainer) { console.error("Fatal Error: App container not found!"); return; }
    updateHeader();
    App.elements.appContainer.innerHTML = `<div class="nav-tabs"><button class="nav-tab active" data-tab="dictionary">Dictionary</button><button class="nav-tab" data-tab="weak-words">Weak Words</button><button class="nav-tab" data-tab="quiz">Quiz</button><button class="nav-tab" data-tab="settings">Settings</button></div><div id="dictionary-tab" class="tab-content active"></div><div id="weak-words-tab" class="tab-content"></div><div id="quiz-tab" class="tab-content"></div><div id="settings-tab" class="tab-content"></div>`;
    renderDictionaryTab();
    renderWeakWordsTab();
    renderQuizTab();
    renderSettingsTab();
...
function updateHeader() {
    ...
    if (App.config.lessonId) {
        ...
        if (lessonInfo) { logoLink.innerHTML = `N5 日本語辞書 <span class="header-lesson-tag">Lesson ${lessonInfo.id}</span>`; }
    } else {
...
function renderDictionaryTab() {
    ...
    container.innerHTML = `<div class="section-box"><h3 style="text-align:center;">Random Word Practice</h3><div class="random-word-container"><div class="flashcard-container"><div id="flashcard-content" class="flashcard-content"><p>Get a random word from the current lesson set, or select words below for a focused study session.</p></div></div><div class="practice-mode-toggle"><span class="lan_toggle">বাংলা → 日本語</span><label class="switch"><input type="checkbox" id="practice-mode-checkbox"><span class="slider"></span></label><span class="lan_toggle">日本語 → বাংলা</span></div><div class="random-word-controls"><button id="get-random-btn" class="control-button">Get Random Word</button><button id="show-meaning-btn" class="control-button" style="display:none;">Show Meaning</button></div></div></div><div class="section-box"><h3>Add New Word</h3><div class="add-word-form"><div class="input-group"><input type="text" id="word-input" placeholder="Bangla Word"></div><div class="input-group"><input type="text" id="meaning-input" placeholder="Japanese Meaning"></div><div class="input-group"><input type="text" id="en-input" placeholder="English Meaning"></div><div class="input-group"><select id="category-select"><option value="">Category</option><option value="Noun">Noun</option><option value="Verb">Verb</option><option value="Adjective">Adjective</option><option value="Adverb">Adverb</option><option value="Phrase">Phrase</option><option value="Particle">Particle</option><option value="Conjunction">Conjunction</option><option value="Counter">Counter</option><option value="Others">Others</option></select></div><div class="add-word-action"><button id="add-word-btn" class="add-button">Add</button></div></div></div><div class="section-box" id="word-list-section"><div class="study-list-controls"><button id="toggle-select-mode-btn" class="control-button">Select for Study</button><div id="selection-actions" style="display: none;"><button id="start-study-btn" class="add-button">Start Practice (<span id="selected-count">0</span>)</button><button id="clear-selection-btn" class="control-button">Clear</button></div></div><div class="dictionary-filters"><select id="category-filter"><option value="">All Categories</option><option value="Noun">Noun</option><option value="Verb">Verb</option><option value="Adjective">Adjective</option><option value="Adverb">Adverb</option><option value="Phrase">Phrase</option><option value="Particle">Particle</option><option value="Conjunction">Conjunction</option><option value="Counter">Counter</option><option value="Others">Others</option></select><input type="search" id="search-input" placeholder="Search all words..."></div><div class="word-list-container" id="word-list-container"></div></div>`;
    document.getElementById('get-random-btn').addEventListener('click', getRandomWord);
    document.getElementById('show-meaning-btn').addEventListener('click', () => toggleRandomMeaning(false));
...
function startStudySession() {
    if (App.config.studyList.length === 0) { alert("Please select at least one word to start a practice session."); return; }
    const encodedWords = encodeURIComponent(App.config.studyList.join('|'));
    const studyUrl = `study.html?words=${encodedWords}`;
    if (window.matchMedia('(display-mode: standalone)').matches) { window.location.href = studyUrl; } else { window.open(studyUrl, '_blank'); }
    toggleSelectionMode(); 
}
document.addEventListener('visibilitychange', () => { if (document.visibilityState === 'visible' && App.config.isSelectionMode) { toggleSelectionMode(); } });
...
function startParticleQuiz() {
    ...
    App.config.currentQuiz = { type: 'particle-quiz', currentQuestionIndex: 0, totalQuestions: parseInt(document.getElementById('quiz-length-select').value, 10), wrongAnswers: [] };
...
async function displayParticleQuestion() {
    ...
    try {
        ...
        if (!matches || matches.length === 0) { displayParticleQuestion(); return; }
        ...
        while (options.length < 4 && otherParticles.length > 0) { options.push(otherParticles.splice(Math.floor(Math.random() * otherParticles.length), 1)[0]); }
...
function checkParticleAnswer(element) {
    ...
    if (isCorrect) {
        ...
        document.querySelectorAll('#quiz-options .quiz-option').forEach(el => { if (el.textContent === answer) el.classList.add('correct'); });
        App.config.currentQuiz.wrongAnswers.push({ sentence: sentence, answer: answer, userChoice: userChoice });
    }
...
async function renderSettingsTab() {
    ...
    // Voice settings are now loaded dynamically
    const voicePlaceholder = document.getElementById('voice-settings-placeholder');
    const getVoices = () => new Promise(resolve => { let voices = window.speechSynthesis.getVoices(); if (voices.length) { resolve(voices); return; } window.speechSynthesis.onvoiceschanged = () => { voices = window.speechSynthesis.getVoices(); resolve(voices); }; });
    ...
    let voiceOptionsHTML = `<option value="google" ${currentVoice === 'google' ? 'selected' : ''}>High Quality (Google)</option>`;
    if (japaneseVoices.length > 0) {
        voiceOptionsHTML += japaneseVoices.map(voice => `<option value="${voice.name}" ${currentVoice === voice.name ? 'selected' : ''}>Basic - ${voice.name} (${voice.lang})</option>`).join('');
        if (!japaneseVoices.some(v => v.name === currentVoice) && currentVoice !== 'google') { voiceOptionsHTML += `<option value="basic_default" selected>Basic (Browser Default)</option>`; } else if (!japaneseVoices.some(v => v.name === currentVoice)) { voiceOptionsHTML += `<option value="basic_default">Basic (Browser Default)</option>`; }
    } else {
...
function attachAppEventListeners() {
    App.elements.appContainer.querySelector('.nav-tabs').addEventListener('click', (e) => {
        if (e.target.matches('.nav-tab')) {
            ...
            if (tabName !== 'dictionary') { clearTimeout(autoRevealTimer); window.removeEventListener('scroll', handleInfiniteScroll); } else { window.addEventListener('scroll', handleInfiniteScroll); }
            if (tabName === 'settings') { renderSettingsTab(); }
            if (tabName === 'weak-words') { renderWeakWordsList(); }
            if (App.config.currentQuiz.type && tabName !== 'quiz') { App.config.quizScore = 0; App.config.currentQuiz = {}; renderQuizTab(); }
...
function getRandomWord() {
    ...
    if (App.config.mainPracticeList.length === 0) {
        ...
        if (wordPool.length === 0) { document.getElementById('flashcard-content').innerHTML = `<p>No words to practice in this lesson set.</p>`; document.getElementById('show-meaning-btn').style.display = 'none'; document.getElementById('get-random-btn').textContent = 'Get Random Word'; return; }
    ...
    if (App.config.practiceMode === 'jp-bn') { const wordData = getWordData(App.config.currentRandomWord); questionText = wordData.meaning; }
    content.innerHTML = `<div class="word-display">${questionText}</div>`;
    const btn = document.getElementById('show-meaning-btn');
    btn.textContent = 'Show Meaning';
    btn.style.display = 'inline-block';
    if (App.config.mainPracticeList.length === 0) { document.getElementById('get-random-btn').textContent = 'Start Over'; }
...
function toggleRandomMeaning(isAutoReveal = false) {
    ...
    if (btn.textContent === 'Show Meaning') {
        ...
        if (App.config.practiceMode === 'jp-bn') { questionText = getWordData(word).meaning; }
        cardContent.innerHTML = `<div class="word-display">${questionText}</div>`;
        btn.textContent = 'Show Meaning';
        if (App.config.autoRevealEnabled) {
            autoRevealTimer = setTimeout(() => {
                if (btn.textContent === 'Show Meaning') { toggleRandomMeaning(true); }
...
function startQuiz(quizType) {
    ...
    if (wordPool.length < 4) { alert('You need at least 4 words in this set to start a quiz!'); return; }
    App.config.quizScore = 0;
    const questions = [];
    ...
    for (let i = 0; i < quizLength && i < shuffledPool.length; i++) {
        ...
        if (quizType === 'jp-to-bangla') { questionText = questionWordData.meaning; correctAnswerText = questionWordKey; } else { questionText = questionWordKey; correctAnswerText = questionWordData.meaning; }
        ...
        questions.push({ word: questionWordKey, question: questionText, answer: correctAnswerText, options: options.sort(() => Math.random() - 0.5) });
    }
    App.config.currentQuiz = { type: quizType, questions: questions, wrongAnswers: [], currentQuestionIndex: 0, totalQuestions: questions.length };
    document.getElementById('quiz-score-container').style.display = 'block';
    document.getElementById('quiz-results-container').style.display = 'none';
    document.getElementById('quiz-score').textContent = '0';
    document.getElementById('total-questions').textContent = App.config.currentQuiz.totalQuestions;
...
function displayQuiz() {
    ...
    const quizContent = document.getElementById('quiz-content');
    quizContent.innerHTML = `<div class="quiz-bangla-word">${currentQuestion.question}</div><div id="quiz-options">${currentQuestion.options.map(o => `<div class="quiz-option">${o}</div>`).join('')}</div>`;
...
function checkAnswer(element) {
    ...
    if (isCorrect) {
        ...
        element.classList.add('wrong');
        document.querySelectorAll('#quiz-options .quiz-option').forEach(el => { if (el.textContent === currentQuestion.answer) el.classList.add('correct'); });
...
function endQuiz() {
    ...
    if (wrongAnswers.length > 0) {
        ...
        if (type.startsWith('type-') || type.startsWith('bangla-') || type.startsWith('jp-')) {
            ...
            resultsHTML += `<div class="word-list-container">${uniqueWrongWords.map(word => createWordCard(word).outerHTML).join('')}</div>`;
        } else if (type === 'particle-quiz') {
            resultsHTML += `<div class="particle-review-container">`;
            wrongAnswers.forEach(item => {
                const highlightedSentence = item.sentence.jp.replace(item.answer, `<strong class="highlight-particle">${item.answer}</strong>`);
                resultsHTML += `<div class="particle-review-item"><p class="sentence-japanese">${highlightedSentence}</p><p class="sentence-bangla">(${item.sentence.bn || item.sentence.en})</p><p class="wrong-answer-info">Your Answer: <span class="wrong-particle">${item.userChoice}</span> | Correct: <span class="correct-particle">${item.answer}</span></p></div>`;
            });
            resultsHTML += `</div>`;
        }
    } else {
...
function openEditModal(word) {
    ...
    const modal = App.elements.modal;
    const toPascalCase = (str) => { if (!str || typeof str !== 'string') return ''; return str.charAt(0).toUpperCase() + str.slice(1).toLowerCase(); };
...
function closeEditModal() { App.elements.modal.style.display = 'none'; }
const audioCache = {};
...
async function speakJapanese(text, forceVoice = null) {
    ...
    if (voicePreference === 'google') {
        if (audioCache[text]) { currentAudio = audioCache[text]; currentAudio.play(); return; }
        try {
            const response = await fetch(`/.netlify/functions/get-google-speech?text=${encodeURIComponent(text)}`);
            if (!response.ok) { const err = await response.json(); throw new Error(err.error || 'Proxy fetch failed'); }
...
function speakWithBasicVoice(text, voiceName) {
    ...
    if (voiceName && voiceName !== 'basic_default') { selectedVoice = voices.find(voice => voice.name === voiceName); } 
    if (!selectedVoice) { selectedVoice = voices.find(voice => voice.lang === 'ja-JP'); }
    if (selectedVoice) { utterance.voice = selectedVoice; }
    window.speechSynthesis.speak(utterance);
}
window.speechSynthesis.onvoiceschanged = () => { window.speechSynthesis.getVoices(); };
function exportData() {
    const userData = { userWords: App.data.userWords, deletedWords: App.data.deletedWords };
...
async function showMnemonic(banglaWord) {
    const wordData = getWordData(banglaWord); 
    if (!wordData || !wordData.en) { alert('No English translation available to search for a mnemonic for this word.'); return; }
    ...
    try {
        ...
        if (!response.ok) { const err = await response.json(); throw new Error(err.error || `API error: ${response.statusText}`); }
        const data = await response.json();
        let imageHtml = `<p class="image-loading-text">No image found for "${englishWord}".</p>`;
        if (data.results && data.results.length > 0) {
            const photo = data.results[0];
            imageHtml = `<a href="${photo.links.html}" target="_blank" rel="noopener noreferrer" class="mnemonic-image-link" cursor="none"><img src="${photo.urls.regular}" alt="${photo.alt_description || 'Visual mnemonic for ' + englishWord}"></a><a href="${photo.user.links.html}" target="_blank" rel="noopener noreferrer" class="pexels-credit">Photo by ${photo.user.name} on Unsplash</a>`;
        }
        modalBody.innerHTML = `<div class="mnemonic-word-info"><div class="mnemonic-bangla">${banglaWord}</div><div class="mnemonic-japanese">${japaneseWord}<span class="speak-icon" onclick="speakJapanese('${japaneseWord}')">🔊</span></div></div>${imageHtml}`;
    } catch (error) {
